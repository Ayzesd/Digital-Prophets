...
📋 Main Functions:
initialize: Sets up the contract and defines an Admin address (who can resolve markets).
create_market: Allows the Admin to start a new betting market.
place_bet:
...
resolve_market: Allows the Admin only to set the final outcome (True/False). This locks the market result.
claim_winnings:
